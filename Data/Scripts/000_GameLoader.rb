...
# ============================================================================
# WICHTIG: Dieses Script muss VOR allen anderen geladen werden!
# Es überschreibt File.read und Marshal.load für transparente Entschlüsselung
# ============================================================================
...
module GameLoader
  ...
  # TODO: Ändere diese Werte für dein finales Release!
  ...
  def self.parse_toc(toc_data, file_count, data_offset)
    ...
    file_count.times do |i|
      ...
      # Read category length - use .ord to get byte value as integer
      category_len = toc_data[offset].ord
      offset = offset.to_i + 1
      
      # Safety check
      if offset + category_len > toc_data.size
        puts "\nFEHLER: Category read would exceed TOC size"
        return false
      end
      ...
      if offset + path_len > toc_data.size
        puts "\nFEHLER: Path read would exceed TOC size"
        return false
      end
      ...
      # Read flags - use .ord for byte value
...
if File.exist?(GameLoader::PACKAGE_FILE)
  ...
  if GameLoader.send(:class_variable_get, :@@package_loaded)
    # Zähle nur einzigartige Dateien (file_table hat jeden Eintrag 2x wegen case-insensitive)
    unique_files = GameLoader.send(:class_variable_get, :@@file_table).size / 2
    puts "✓ Encrypted game data loaded (#{unique_files} files)"
  end
end
